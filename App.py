...
def classify_and_extract(text):
    encodings = tokenizer(text, truncation=True, padding=True, max_length=100, return_tensors="pt")
    with torch.no_grad():
...
if st.button("Extract Countries"):
    if user_input:
        ...
        for sentence in sentences:
            countries = classify_and_extract(sentence.strip())
            if countries:
                result.append((sentence, countries))

        if result:
            st.write("**Extracted Countries:**")
            for sentence, countries in result:
                st.write(f"Sentence: {sentence}")
                st.write(f"Countries: {', '.join(countries)}")
        else:
            st.write("No countries were detected in the text.")
    else:
